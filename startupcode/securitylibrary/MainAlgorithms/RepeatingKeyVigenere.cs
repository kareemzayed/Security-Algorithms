...
namespace SecurityLibrary
{
    public class RepeatingkeyVigenere : ICryptographicTechnique<string, string>
    {
        public string Analyse(string plainText, string cipherText)
        {
            ...
            {
                if (keystream[z] == keystream[counter_1] && z != counter_1)
                {
                    if (z + 1 < keystream.Length && z + 2 < keystream.Length)
                    {
                        if (keystream[counter_1 + 1] == keystream[z + 1] && keystream[counter_1 + 2] == keystream[z + 2])
                        {
                            index1 = z;
                            break;
                        }
